...
[`MemoryEntry`](Storage-Primitives) (`src/memory/types.rs`):
...
> [Sources](Sources) for where `ExternalSync` originates.
...
reliably runnable implementation today — see [Getting Started](Getting-Started).
...
disables recency boosting for that profile. See [Retrieval](Retrieval) for how
profiles are selected per query.

### Decay and reinforcement

TinyCortex applies a **time-decay model** inspired by the Ebbinghaus Forgetting
Curve (documented in `gitbooks/developers/concepts/memory-decay.md`):
...
alongside structured chunks (`gitbooks/developers/concepts/recall.md`). Within
**this crate**, the closest real type is `NamespaceRetrievalContext`, which pairs
a rendered `context_text` with the ranked `hits` that back it:
