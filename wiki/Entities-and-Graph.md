...
- [Scoring-and-Extraction](Scoring-and-Extraction) — where entity kinds and
  canonical ids are emitted during ingest.
- [Summary-Trees](Summary-Trees) — the tree nodes the occurrence index is keyed
  on.
- [Storage-Primitives](Storage-Primitives) — the content store and derived
  indexes the registry and graph build on.
- [Core-Concepts](Core-Concepts) — provenance, taint, and the layer boundaries
